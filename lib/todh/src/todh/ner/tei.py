...
import todh.ner.utils as ner_utils


def tei_ner_params(contenu, racine, balise, moteur, modele, filtre=None, encodage="utf-8"):
    pipeline, label_function, iterator = ner_utils.load(moteur, modele)
...
def run(
    ...
    pipeline, label_function, iterator = ner_utils.load(annotateur, modele)
